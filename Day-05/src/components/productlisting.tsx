...
const ProductListing = () => {
  ...
  const [cartItems, setCartItems] = useState(0); // To track items in cart
      ...
      <div className="px-4 md:px-8 lg:px-12 py-8 md:py-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-8">
          {['chair', 'vase', 'silky', 'lamp'].map((item, idx) => (
            <div key={idx} className="w-full group relative">
              <Image
                src={`/images/${item}.png`}
                height={700}
                width={700}
              ...
              <div className="absolute inset-0 bg-[#00000099] opacity-0 group-hover:opacity-100 flex justify-center items-center text-white transition-opacity duration-300">
                <button className="bg-[#2A254B] px-4 py-2 rounded-md">View Product</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="my-10 flex justify-center">
          <button 
            className="bg-[#d4d0d0] py-3 px-6 rounded-md text-[#2A254B]"
            onClick={() => alert('Redirecting to full collection')}
          >
            View collection
          </button>
        </div>
